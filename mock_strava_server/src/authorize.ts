...
export function authorize(req: Request, res: Response) {
  ...
  if (clientId !== 'strava-client-id') {
    console.error(`[authorize] Invalid client_id: "${clientId}", expected "strava-client-id"`);
    res.status(400).json({ error: `Invalid client_id: "${clientId}"` });
    return;
  }

  if (scope !== 'activity:read') {
    console.error(`[authorize] Invalid scope: "${scope}", expected "activity:read"`);
    res.status(400).json({ error: `Invalid scope: "${scope}", expected "activity:read"` });
    return;
  }
