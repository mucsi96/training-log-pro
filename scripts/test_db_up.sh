#!/bin/bash
set -e

PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
POD_NAME="training-log-pro-test-db"
DB_CONTAINER="training-log-pro-test-db-db"
MAX_WAIT=60

if podman inspect --format='{{.State.Running}}' "$DB_CONTAINER" 2>/dev/null | grep -q "true"; then
  echo "Test database is already running"
  exit 0
fi

echo "Cleaning up existing pod..."
podman kube down "$PROJECT_DIR/server/test-db-pod.yaml" 2>/dev/null || true

echo "Starting test database pod..."
podman kube play "$PROJECT_DIR/server/test-db-pod.yaml"

echo "Waiting for database to become healthy..."
ELAPSED=0
while true; do
  if [ "$ELAPSED" -ge "$MAX_WAIT" ]; then
    echo "Timeout waiting for database to become healthy"
    exit 1
  fi
  STATUS=$(podman inspect --format='{{.State.Health.Status}}' "$DB_CONTAINER" 2>/dev/null || echo "missing")
  if [ "$STATUS" = "healthy" ]; then
    echo "Test database is ready on port 5481"
    break
  fi
  sleep 2
  ELAPSED=$((ELAPSED + 2))
done
