...
# code cache and no JIT-compiled code to hold, and it idles below what a JRE
# image would use. The request is what the scheduler reserves around the
# clock, so it is sized for idle: metrics-server reports a ~95Mi working set
# in production - far below the ~275MB a glibc build of the same source
# measured outside Kubernetes, because the musl binary actually deployed is
# much smaller - so the request sits just above that measurement.
#
